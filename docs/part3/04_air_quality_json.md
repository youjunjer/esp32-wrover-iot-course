...
6. 來源時間不得超前本機已同步時間 5 分鐘以上，也不得老於 2 小時。
...
成功畫面同時顯示資料來源時間 `SRC`、由本機已同步時間計算的 `AGE`，以及下一次輪詢倒數。若後續抓取失敗但記憶體內仍有前次資料，OLED 必須標成 `AQ DATA STALE` 並保留錯誤，不可只顯示舊數字讓人誤以為剛更新。
