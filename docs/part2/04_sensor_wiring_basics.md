...
有些模組需要 5V 供電，但它的輸出也可能接近 5V。ESP32 GPIO 不是 5V tolerant；在不知道輸出電壓前，不可直接接入。新版教材採以下兩種作法：

- MQ-2：`AO → 10 kΩ → GPIO 33`，再由 `GPIO 33 → 12 kΩ → GND`。名目 5.0V 經分壓約為 `2.73V`。
- HC-SR04：使用 1% 電阻，`Echo → 10 kΩ → GPIO 14`，再由 `GPIO 14 → 15 kΩ → GND`。名目 5.0V 經分壓約為 `3.00V`，比貼近 3.3V 的設計保留更多供電與電阻誤差裕量；也可改用符合訊號方向與速率的位準轉換器。

分壓值只是一項設計計算，不能代替量測。接上 ESP32 前，先讓模組獨立供電並以電表確認分壓節點在預期範圍；模組批次、供電誤差與電阻容差都可能改變實際電壓。

1602 的 PCF8574 I²C 背板則要另外檢查 SDA／SCL 上拉電壓。常見背板以 5V 供電時，會把兩條 I²C 線拉到 5V；此時不能直接與 ESP32 共用 GPIO 21／22。請使用已確認可在 3.3V 工作的背板，或加入雙向 I²C 位準轉換器。普通單向邏輯轉換器不一定適用 I²C 的雙向開漏訊號。
