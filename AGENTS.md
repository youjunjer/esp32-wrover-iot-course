...
- CLI、網頁或工具操作畫面使用真實截圖；不得在繪圖工具中重打成功訊息並冒充執行結果。
- 預期 OLED 畫面或 DEMO 資料必須直接在圖內與圖說標示「非實機照片／非實測資料」。
- 操作截圖的來源記錄應包含工具與版本、指令或 CI Run、Commit、處理方式、證據邊界及 SHA-256；不得露出憑證、本機帳號、完整私人路徑或裝置識別資訊。
