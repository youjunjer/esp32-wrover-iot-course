...
三個色彩通道的最高 PWM duty 限制為 `96/255`，避免初次接線時過亮；限流電阻仍不可省略。

## 元件

- 四腳共陰極 RGB LED 一顆。
- `220Ω`～`330Ω` 限流電阻三個；每個色彩通道各使用一個，不能只在共用腳放一個電阻。
...
這組腳位延續 AB143 程式中的 R15／G2／B4；舊版教材的部分接線文字與程式不一致，本範例已統一。GPIO 2 與 15 是啟動綁定腳位，請依表格使用 LED 與限流電阻作為輸出負載，不要在開機期間用外部電路強制拉高或拉低。改動接線前先斷電。
